...
def send_peer_data(peer_socket, peer_id, filename, data_dict):
    ...
    try:
        ...
        num_chunks = (len(message) // 1024) + 1
        peer_socket.send(num_chunks.to_bytes(4, byteorder='big'))

        # Send the data in chunks
        for i in range(0, len(message), 1024):
            chunk = message[i:i+1024]
            peer_socket.send(chunk.encode('utf-8'))
    ...
    try:
        ...
        for peer_ip, port in peer_ip_ports:
            ...
            for i in range(1, 6):
                ...
                csv_files = glob.glob(os.path.join("/users/pgrad/singhr6/group24/data/Satellite1", current_date, current_hour, "*.csv"))
                ...
                if csv_files:
                    ...
                    print("No CSV files found for Satellite1 Date: " + current_date + " Hour: " + current_hour)
            
                csv_files = glob.glob(os.path.join("/users/pgrad/singhr6/group24/data/Satellite2", current_date, current_hour, "*.csv"))
                ...
                if csv_files:
                    ...
                    print("No CSV files found for Satellite2 Date: " + current_date + " Hour: " + current_hour)
